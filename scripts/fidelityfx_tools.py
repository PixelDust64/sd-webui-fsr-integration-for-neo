...
# Garante compatibilidade com Forge Neo e A1111
try:
...
def getFidelityFXEXE():
    if not os.path.exists(fidelityFX_exe):
        ...
        if os.path.exists(fidelityFX_zip):
...
def runFidelityFX(img: Image.Image, scale=None, resize_x=None, resize_y=None) -> Image.Image:
    ...
    try:
        ...
        runFidelityFX_(scale, fileIn, fileOut, resize_x, resize_y)
        ...
        print(f"Error running FidelityFX: {e}")
        return img
    finally:
        try:
            tmpInDir.cleanup()
